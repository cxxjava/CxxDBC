...
static void test_db_connect() {
	...
	//2.
	conn.connect("edbc:MSSQL://" HOST ":" PORT "/" DATABASE "?connectTimeout=10", USERNAME, PASSWORD);
	ON_FINALLY_NOTHROW(
		conn.close();
	) {
		EDatabaseMetaData* meta = conn.getMetaData();
...
static void test_db_execute() {
	...
	//0.
	try {
		stmt.setSql("DROP TABLE mssql001").execute();
	} catch (...) {
	}
	stmt.setSql("CREATE TABLE mssql001 ("
            "a int NULL ,"
            "b tinyint NULL ,"
            "c binary (50) NULL ,"
            "d bit NOT NULL ,"
	...
	stmt.setSql("insert into mssql001 (d,c,a,i,f,h,g,m,e) values(1,0x12345000,?,?,?,8888.88,?,?,?)")
	...
	//2.
	stmt.clear();
	stmt.setSql("select * from mssql001").execute();
...
//	//3.
	stmt.clear();
	stmt.setFetchSize(2);
	stmt.setSql("select * from mssql001 where a=? or i=?")
			.bindInt(4)
			.bindString("1")
			.execute();
	rs = stmt.getResultSet();
...
static void test_db_update() {
	...
	stmt.setSql("insert into mssql001 (d,a,i,f) values (1, -2, '666', '2015-5-5')"); //1
	stmt.addBatch("insert into mssql001 (d,a,i,f) values (?, ?, ?, ?)"); //2
	stmt.bindBool(true);
	stmt.bindInt(-1);
	stmt.bindString("777");
	stmt.bindString("2016-6-6");
	stmt.addBatch("insert into mssql001 (d,a,i,f) values (0, ?, ?, ?)"); //3
		stmt.bindInt(-3);
		stmt.bindString("333");
		stmt.bindString("2016-6-6");
	for (int i = 0; i < 99; i++) {
	...
	stmt.addBatch("insert into mssql001 (d,a,i,f) values (0, -2, '666', '2015-5-5')"); //103
	stmt.addBatch("insert into mssql001 (d,a,i,f) values (0, -2, '666', '2015-5-5')"); //104
	stmt.execute();
...
static void test_db_commit() {
	...
	ECommonStatement stmt(&conn);
	stmt.setSql("update mssql001 set i='xxxxxxxxxx' where a=1");
...
static void test_db_rollback() {
	...
	ECommonStatement stmt(&conn);
	try {
		stmt.setSql("drop table mssql002").execute();
	} catch (...) {
	}
	stmt.setSql("create table mssql002 (a int)").execute();
	conn.setAutoCommit(false);
	ESavepoint sp1 = conn.setSavepoint();
	stmt.setSql("insert into mssql002 values (1)").execute();
//	conn.rollback();
//	conn.setAutoCommit(true);
	ESavepoint sp2 = conn.setSavepoint();
	stmt.setSql("insert into mssql002 values (2)").execute();
	conn.rollback(sp2);
	conn.commit();
...
static void test_create_table() {
	...
	stmt.addBatch("drop table mssql001");
	stmt.addBatch("CREATE TABLE mssql001 ("
            "a int NULL ,"
            "b tinyint NULL ,"
            "c binary (50) NULL ,"
            "d bit NOT NULL ,"
...
static void test_sql_insert() {
	...
	stmt.addBatch("insert into mssql001 (d,e,g) values (1,?,?)");
	...
	stmt.addBatch("insert into mssql001 (a,b,c,d,e,f,g,i) values (?,?,?,0,?,?,?,?)");
	stmt.bindInt(55555);
	stmt.bindShort(12);
...
static void test_sql_update() {
	...
	//1.
	stmt.setSql("update mssql001 set n=?,k=?,a=?,d=?,g=?,m=?");
	...
	//2. text: <=64k
	stmt.addBatch("update mssql001 set e=?");
	es_buffer_t *buffer = eso_buffer_make(100, 0);
	...
	//3. image: <=64k
	stmt.addBatch("update mssql001 set l=?");
...
static void test_sql_func() {
	...
	//create
	{
		EUpdateStatement stmt(&conn);
		stmt.setFailedResume(true);
		stmt.addBatch("drop procedure mssql001func");
		stmt.addBatch("create procedure mssql001func "
                "@a int "
                "as select * from mssql001 where a=@a "
                "select a,b,c,d,e from mssql001");
		stmt.execute();
	}
	...
	//select
	{
		...
		stmt.setSql("exec mssql001func @a=?");
		stmt.bindInt(4);
#else
		stmt.setSql("exec mssql001func @a=4");
#endif
		stmt.execute();
		EResultSet* rs = stmt.getResultSet();
		while (rs != null) {
	...
	for (int i = 0; i < 20; i++) {
		sp<EThread> ths = EThread::executeX([&]() {
			...
			for (int j = 0; j < 1000; j++) {
				...
				stmt->setSql("update mssql002 set a=1").execute();
...
void test_db_mssql(void)
{
	EConnection::setDefaultDBType("MSSQL");
